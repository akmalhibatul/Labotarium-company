        ...
        <div class="content content-full">
            <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
                <h1 class="flex-sm-fill h3 my-2">
                    Edit Test <small class="d-block d-sm-inline-block mt-2 mt-sm-0 font-size-base font-w400 text-muted"></small>
                </h1>
    ...
    <div class="content">
        ...
        <div class="block">
            <div class="block-content block-content-full">
                <form action="<?= base_url() ?>admin/updateTest" method="POST" enctype="multipart/form-data" novalidate>
                    <input type="text" name="id" value="<?= $test['id']; ?>" hidden>
                    <div class="row push">
                        <div class="col-lg-12 col-xl-12">
                            <div class="form-group">
                                <label for="example-text-input">Nama Test</label>
                                <input type="text" class="form-control form-control-alt" id="example-text-input" name="nama" value="<?= $test['nama']; ?>">
                                <small class="form-text text-danger"><?= form_error('nama'); ?></small>
                            </div>
                            <div class="form-group">
                                <label for="example-text-input">Detail</label>
                                <textarea name="detail" class="form-control form-control-alt" rows="4"><?= $test['detail']; ?></textarea>
                            ...
                            <div class="form-group">
                                ...
                                <select name="id_kategori" class="custom-select" required>
                                    ...
                                    <?php foreach ($kt as $k) : ?>
                                        <?php
                                        if ($k['id_kategori'] == $test['id_kategori']) {
                                            $select = "selected";
                                        } else {
                                            $select = "";
                                        }
                                        echo "<option $select value='" . $k['id_kategori'] . "'>" . $k['nama_kategori'] . "</option>";
                                        ?>
                                    <?php endforeach ?>
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select name="type" class="custom-select" required>
                                    ...
                                    <option <?= ($type == 'umum') ? "selected" : "" ?> value="umum">umum</option>
                                    <option <?= ($type == 'covid') ? "selected" : "" ?> value="covid">covid</option>
                                </select>
                                <small class="form-text text-danger"><?= form_error('type'); ?></small>
                            </div>
                        </div>
